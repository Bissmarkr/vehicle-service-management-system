...
function PaymentState({ booking, invoice, onPay, onRemainingPay, paying }) {
  const payment = booking?.advance_payment || invoice?.advance_payment;
  const paymentStatus = booking?.payment_status || invoice?.payment_status || invoice?.advance_payment_status;
  const adminStatus = booking?.admin_status || invoice?.admin_status || payment?.admin_status;
  const advanceRequired = Number(invoice?.advance_amount || 0) > 0;
  const remainingAmount = Number(booking?.remaining_amount ?? invoice?.remaining_amount ?? 0);
  const remainingStatus = String(booking?.remaining_payment_status || invoice?.remaining_payment_status || '').toUpperCase();
  const advancePaid = paymentIsPaid(payment?.payment_status || paymentStatus);
  const details = payment && <small className="payment-details">Amount: {money(payment.payment_amount || invoice?.advance_amount)} | Date: {formatDate(payment.payment_date)} | Method: {payment.payment_method || 'CARD'}</small>;

  if (advancePaid && (remainingStatus === 'PAID' || remainingAmount <= 0)) return <div className="payment-state"><strong className="payment-status">Fully Paid ✓</strong>{details}</div>;
  if (advancePaid && adminStatus === 'REJECTED') return <div className="payment-state"><strong className="payment-status">Payment Rejected</strong><small className="payment-details">{payment?.rejection_reason || invoice?.rejection_reason || 'Please contact the service center.'}</small>{advanceRequired && <button className="neon-button" onClick={() => onPay(invoice.invoice_id)}>Pay Advance Again</button>}{details}</div>;
  if (advancePaid && remainingAmount > 0) return <div className="payment-state"><strong className="payment-status">{adminStatus === 'APPROVED' ? 'Advance Payment Approved ✓' : 'Advance Payment Completed ✓'}</strong><button className="neon-button pending-payment-button" onClick={() => onRemainingPay?.(booking?.booking_id || invoice?.booking_id)}>Pending Payment</button>{details}</div>;
  if (advancePaid) return <div className="payment-state"><strong className="payment-status">Payment Submitted ✓</strong><small className="payment-details">Waiting for Admin Approval</small>{details}</div>;
  return advanceRequired ? <button className="neon-button" disabled={paying === invoice?.invoice_id} onClick={() => onPay(invoice?.invoice_id)}>{paying === invoice?.invoice_id ? 'Processing...' : 'Pay Advance'}</button> : <span>No advance required</span>;
}
...
function CustomerDashboard({ auth, logout }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (modal.type === 'vehicle') {
        ...
        setModal(null);
        navigate(`/booking-confirmation/${response.data.booking.booking_id}`);
      }
      await load();
  ...
  const payInvoice = async (invoiceId) => {
    if (!invoiceId) { setError('No advance invoice is available for this booking.'); return; }
    setPaying(invoiceId);
    setError('');
    try {
    ...
    <main className="customer-main"><header className="customer-topbar"><button className="mobile-menu" onClick={() => setMobileNav(!mobileNav)} aria-label="Open menu"><Menu size={20} /></button><div className="customer-search"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search your vehicles, bookings or invoices" /></div><div className="customer-top-actions"><button className="icon-button" onClick={() => setMessage(data?.notifications?.length ? `${data.notifications.length} booking updates available.` : 'No new notifications.')} aria-label="Notifications"><Bell size={18} />{data?.notifications?.length > 0 && <i />}</button><div className="customer-profile"><div className="avatar">{customer.name?.slice(0, 1).toUpperCase() || 'C'}</div><div><strong>{customer.name || 'Customer'}</strong><span>Customer account</span></div></div></div></header>
      <div className="customer-content"><div className="customer-hero" id="customer-overview"><div><span className="eyebrow"><Zap size={13} /> CUSTOMER CONTROL CENTER</span><h1>Welcome back, <em>{customer.name?.split(' ')[0] || 'there'}</em></h1><p>Keep your vehicles moving with a clearer view of every service moment.</p><div className="hero-actions"><button className="neon-button" onClick={openBooking}><Plus size={17} /> Book a service</button><button className="ghost-button" onClick={() => document.getElementById('customer-vehicles')?.scrollIntoView({ behavior: 'smooth' })}>View vehicles <ChevronRight size={16} /></button></div></div><div className="hero-orbit"><div className="orbit-ring ring-one" /><div className="orbit-ring ring-two" /><CarFront size={76} strokeWidth={1.1} /></div></div>
        ...
        <div className="customer-grid lower-grid"><section className="glass-panel" id="customer-history"><div className="panel-heading"><div><span className="eyebrow">MAINTENANCE LOG</span><h2>Recent history</h2></div><History size={20} className="panel-icon" /></div>{(data?.service_history || []).length ? data.service_history.map((item) => <div className="history-row" key={item.history_id}><div className="history-icon"><Wrench size={17} /></div><div><strong>{item.booking?.service?.service_name || item.service_summary || 'Service visit'}</strong><span>{item.vehicle?.registration_number || '-'} &middot; {formatDate(item.service_date)}</span></div><StatusPill status={item.final_status || 'COMPLETED'} /></div>) : <p className="muted-copy">Completed service history will appear here.</p>}</section>
          <section className="glass-panel" id="customer-invoices"><div className="panel-heading"><div><span className="eyebrow">BILLING</span><h2>Recent invoices</h2></div><FileText size={20} className="panel-icon" /></div>{(data?.invoices || []).length ? data.invoices.map((invoice) => <article className="invoice-item" key={invoice.invoice_id}><button className="invoice-icon" onClick={() => printInvoice(invoice)} aria-label="Print invoice"><FileText size={17} /></button><div className="invoice-info"><div className="invoice-top-row"><strong className="invoice-number">Invoice #{String(invoice.invoice_id).padStart(4, '0')}</strong><strong className="invoice-amount">{money(invoice.total_amount)}</strong></div><div className="service-name">{invoice.booking?.service?.service_name || 'Service'}</div><PaymentState invoice={invoice} onPay={payInvoice} onRemainingPay={(bookingId) => navigate(`/customer/pending-payment/${bookingId}`)} paying={paying} /></div></article>) : <p className="muted-copy">Your invoices will appear after a service is billed.</p>}</section></div>
